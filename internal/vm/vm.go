...
func (vm *Vm) metaValue(typ funds.MetaValueType, reg byte) funds.MetaValue {
	var value string
	switch typ {
	...
	return funds.MetaValue{Value: value, Typ: typ}
}
...
func Exec[S Store](
	...
	var txMeta map[string]funds.MetaValue
	...
	for pc < instructionsLen {
		...
		case Op_Restore:
			...
			if !snap.IsInt64() {
				return funds.ExecutionResult{}, InternalError{Err: fmt.Errorf("invalid snapshot id %s", snap.String())}
			}
		...
		case Op_SetTxMeta:
			if txMeta == nil {
				txMeta = map[string]funds.MetaValue{}
			}
			txMeta[stringsRegs[instr.A]] = vm.metaValue(funds.MetaValueType(instr.C), instr.B)
		...
		case Op_SetAccountMeta:
			...
			accMeta[stringsRegs[instr.B]] = vm.metaValue(funds.MetaValueType(instrExt.A), instr.C)
