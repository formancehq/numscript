...
func portion(num, den int64) funds.Portion {
	return funds.Portion{Num: *big.NewInt(num), Den: *big.NewInt(den)}
}

// allot fills a fresh buffer via MakeAllotment and returns it, for ergonomics.
func allot(amount int64, portions []funds.Portion) []big.Int {
	out := make([]big.Int, len(portions))
	funds.New(nil).MakeAllotment(out, big.NewInt(amount), portions)
	return out
}
...
func TestMakeAllotment_EvenSplit(t *testing.T) {
	wantParts(t, allot(100, []funds.Portion{portion(1, 2), portion(1, 2)}), []int64{50, 50})
}

func TestMakeAllotment_UnevenSplit(t *testing.T) {
	wantParts(t, allot(100, []funds.Portion{portion(1, 4), portion(3, 4)}), []int64{25, 75})
}
...
func TestMakeAllotment_RemainderGoesToEarliest_Thirds(t *testing.T) {
	...
	wantParts(t, allot(100, []funds.Portion{portion(1, 3), portion(1, 3), portion(1, 3)}), []int64{34, 33, 33})
}

func TestMakeAllotment_RemainderTwoUnits(t *testing.T) {
	// 1/6,1/6,4/6 of 100 -> 16,16,66 (sum 98); leftover 2 -> first two get +1.
	wantParts(t, allot(100, []funds.Portion{portion(1, 6), portion(1, 6), portion(4, 6)}), []int64{17, 17, 66})
}

func TestMakeAllotment_HalvesOfOddAmount(t *testing.T) {
	// 7 split in half -> 3,3 (sum 6); leftover 1 -> first.
	wantParts(t, allot(7, []funds.Portion{portion(1, 2), portion(1, 2)}), []int64{4, 3})
}

func TestMakeAllotment_SinglePortionWhole(t *testing.T) {
	wantParts(t, allot(100, []funds.Portion{portion(1, 1)}), []int64{100})
}

func TestMakeAllotment_ZeroAmount(t *testing.T) {
	wantParts(t, allot(0, []funds.Portion{portion(1, 3), portion(2, 3)}), []int64{0, 0})
}

func TestMakeAllotment_EmptyPortions(t *testing.T) {
	out := []big.Int{}
	funds.New(nil).MakeAllotment(out, big.NewInt(100), []funds.Portion{})
	if len(out) != 0 {
...
func TestMakeAllotment_PercentageLikePortions(t *testing.T) {
	...
	wantParts(t, allot(10_000, []funds.Portion{portion(19, 100), portion(81, 100)}), []int64{1900, 8100})
}
...
func TestMakeAllotment_UnreducedPortionsSameAsReduced(t *testing.T) {
	...
	wantParts(t, allot(100, []funds.Portion{portion(2, 4), portion(2, 4)}), []int64{50, 50})
}
...
func TestMakeAllotment_PartsAlwaysSumToAmount(t *testing.T) {
	...
	out := make([]big.Int, 3)
	funds.New(nil).MakeAllotment(out, amount, []funds.Portion{portion(1, 7), portion(2, 7), portion(4, 7)})
	sum := new(big.Int)
	for i := range out {
		sum.Add(sum, &out[i])
	}
...
func TestMakeAllotment_BeyondInt64(t *testing.T) {
	...
	out := make([]big.Int, 2)
	funds.New(nil).MakeAllotment(out, amount, []funds.Portion{portion(1, 2), portion(1, 2)})
...
func TestMakeAllotment_ModifiesCallerSliceAndOverwritesStale(t *testing.T) {
	...
	out[0].SetInt64(999)
	out[1].SetInt64(-7)
	funds.New(nil).MakeAllotment(out, big.NewInt(100), []funds.Portion{portion(1, 4), portion(3, 4)})
	wantParts(t, out, []int64{25, 75})
}

func TestMakeAllotment_DoesNotMutateInputs(t *testing.T) {
	portions := []funds.Portion{portion(1, 3), portion(2, 3)}
	amount := big.NewInt(100)
	out := make([]big.Int, 2)
	funds.New(nil).MakeAllotment(out, amount, portions)
