...
func BenchmarkRuntimeBaselineUnique(b *testing.B) {
	store := fundsStoreAdapter{store: benchStore{balances: map[funds.PairKey]*big.Int{}}}
	rs := funds.New(store)
...
func BenchmarkCompiledVMOpt(b *testing.B) {
	benchCompiledVMOpt(b, benchSrc, benchStore{balances: map[funds.PairKey]*big.Int{
		{Account: "src", Asset: "USD/2", Color: ""}: big.NewInt(100),
	}})
}
...
func BenchmarkCompiledVMOptAllotment(b *testing.B) {
	benchCompiledVMOpt(b, benchSrcAllotment, benchStore{balances: map[funds.PairKey]*big.Int{
...
func BenchmarkCompiledVMCold(b *testing.B) {
	benchCompiledVMCold(b, benchSrc, benchStore{balances: map[funds.PairKey]*big.Int{
		{Account: "src", Asset: "USD/2", Color: ""}: big.NewInt(100),
	}}, false)
}
