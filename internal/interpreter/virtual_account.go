...
type VirtualAccount struct {
	Dbg     string
	credits map[string]*fundsQueue
	debits  map[string]*fundsQueue
}
...
func (vacc *VirtualAccount) getCredits(asset string) *fundsQueue {
	return utils.MapGetOrPutDefault(vacc.credits, asset, func() *fundsQueue {
		fs := newFundsQueue(nil)
		return &fs
	})
}

func (vacc *VirtualAccount) getDebits(asset string) *fundsQueue {
	return utils.MapGetOrPutDefault(vacc.debits, asset, func() *fundsQueue {
		fs := newFundsQueue(nil)
		return &fs
	})
}
...
func repayWithSender(s *fundsQueue, asset string, credit Sender) ([]Posting, *big.Int) {
