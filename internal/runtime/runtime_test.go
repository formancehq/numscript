...
func (m *mockStore) GetBalance(account, asset, color string) int64 {
...
func TestSend_MergesWithinSingleDrain(t *testing.T) {
	...
	rs, _ := newRS(map[runtime.PairKey]int64{{"A", usd, ""}: 100})
	rs.Pull("A", 60, runtime.BoundedOverdraft(0), "") // source A:60
	rs.Pull("A", 40, runtime.BoundedOverdraft(0), "") // source A:40
	rs.Send(strptr("X"), 100, "")                     // drains both A:60 then A:40 -> one posting
	wantPostings(t, rs, []runtime.Posting{{Source: "A", Destination: "X", Asset: usd, Amount: 100}})
}
...
func TestSend_DoesNotMergeAcrossSeparateSends(t *testing.T) {
	...
	rs.Send(strptr("X"), 40, "") // separate send: NOT merged, requeue A:20
	wantPostings(t, rs, []runtime.Posting{
...
func TestColor_BalancesTrackedSeparatelyPerColor(t *testing.T) {
	...
	if got := rs.GetAccountBalance("A", usd, "red"); got != 100 {
		t.Errorf("red balance = %d, want 100", got)
	}
	if got := rs.GetAccountBalance("A", usd, "blue"); got != 40 {
		t.Errorf("blue balance = %d, want 40", got)
	}
	// uncolored slot is independent and absent -> 0
	if got := rs.GetAccountBalance("A", usd, ""); got != 0 {
...
func TestColor_PullTagsSourceAndPostingCarriesColor(t *testing.T) {
	...
	rs.Pull("A", 60, runtime.BoundedOverdraft(0), "red")
	rs.Send(strptr("X"), 60, "red")
	wantPostings(t, rs, []runtime.Posting{
		{Source: "A", Destination: "X", Asset: usd, Color: "red", Amount: 60},
	})
	// destination credited on the colored slot, source debited on it
	if got := rs.GetAccountBalance("X", usd, "red"); got != 60 {
		t.Errorf("X red = %d, want 60", got)
	}
	if got := rs.GetAccountBalance("A", usd, "red"); got != 40 {
...
func TestColor_SendSkipsNonMatchingColorLeavingItQueued(t *testing.T) {
	...
	rs.Pull("A", 50, runtime.BoundedOverdraft(0), "red")
	rs.Pull("B", 30, runtime.BoundedOverdraft(0), "blue")
	rs.Pull("C", 40, runtime.BoundedOverdraft(0), "red")

	rs.Send(strptr("X"), 100, "red") // only 90 red available; blue stays put
	wantPostings(t, rs, []runtime.Posting{
		{Source: "A", Destination: "X", Asset: usd, Color: "red", Amount: 50},
		{Source: "C", Destination: "X", Asset: usd, Color: "red", Amount: 40},
	})

	// the skipped blue source is still queued and drains on a blue send
	rs.Send(strptr("Y"), 100, "blue")
	wantPostings(t, rs, []runtime.Posting{
		{Source: "A", Destination: "X", Asset: usd, Color: "red", Amount: 50},
		{Source: "C", Destination: "X", Asset: usd, Color: "red", Amount: 40},
		{Source: "B", Destination: "Y", Asset: usd, Color: "blue", Amount: 30},
	})
}
...
func TestColor_SendDoesNotMergeAcrossColors(t *testing.T) {
	...
	rs.Pull("A", 40, runtime.BoundedOverdraft(0), "red")
	rs.Pull("A", 40, runtime.BoundedOverdraft(0), "blue")
	rs.SendUncapped(strptr("X"), "red")
	rs.SendUncapped(strptr("X"), "blue")
	wantPostings(t, rs, []runtime.Posting{
		{Source: "A", Destination: "X", Asset: usd, Color: "red", Amount: 40},
		{Source: "A", Destination: "X", Asset: usd, Color: "blue", Amount: 40},
	})
}

func TestColor_RefundUsesSourceColor(t *testing.T) {
	rs, _ := newRS(map[runtime.PairKey]int64{{"A", usd, "red"}: 100})
	rs.Pull("A", 100, runtime.BoundedOverdraft(0), "red") // A red -> 0
	rs.Send(nil, 60, "red")                               // refund 60 to A's red slot
	if got := rs.GetAccountBalance("A", usd, "red"); got != 60 {
